...
// HandleGetSliTriggeredEvent handles get-sli.triggered events if SLIProvider == skywalking-service
...
func HandleGetSliTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.GetSLITriggeredEventData) error {
	...
	if data.GetSLI.SLIProvider != "skywalking-service" {
	...
	// Get SLI File from skywalking-service subdirectory of the config repo - to add the file use:
	//   keptn add-resource --project=PROJECT --stage=STAGE --service=SERVICE --resource=my-sli-config.yaml  --resourceUri=skywalking-service/sli.yaml
	sliFile := "skywalking-service/sli.yaml"
